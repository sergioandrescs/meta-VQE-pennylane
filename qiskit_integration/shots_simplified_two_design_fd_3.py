...
with open('./ibm_lagos_model_error.json', 'r') as reader:
...
num_params = (WIRES + LAYERS*(WIRES-1)*2)

num_params_encoding = (WIRES + LAYERS*(WIRES-1)*2)*ENCODING_MULTIPLIER
...
def core_circuit(params):
    ...
    init_weights, weights = np.reshape(
        params[:qubits], shapes[0]), np.reshape(params[qubits:], shapes[1])

    qml.SimplifiedTwoDesign(
        initial_layer_weights=init_weights, weights=weights, wires=range(WIRES))
...
for num_run in range(RUNS):
    ...
    with open(f'exp_runs/simplified2design_{LAYERS}layers_{GRADIENT_METHOD}_{num_run}.json', 'w') as f:
        f.write(json.dumps({
            "layers": LAYERS,
