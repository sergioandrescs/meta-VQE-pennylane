...
def gradient_descent(fun, x0, stepsize=0.1, tol=1e-4, maxiter=100,  verbose=False, hamiltonian=None, **options):

    new_params = np.array(x0)
    ref_energy, grad = fun(new_params, hamiltonian)
    ...
    for i in range(maxiter):
        ...
        new_energy, grad = fun(new_params, hamiltonian)
...
def spsa_optimizer(fun, x0, maxiter=100, alpha=0.602, gamma=0.101, c=0.2, A=None, a=None, tol=1e-4, verbose=False, hamiltonian=None, **options):
    new_params = np.array(x0)
    ref_energy = fun(new_params, hamiltonian, return_grad=False)
    ...
    for i in range(maxiter):
        ...
        yplus = fun(thetaplus, hamiltonian, return_grad=False)
        yminus = fun(thetaminus, hamiltonian, return_grad=False)

        grad = np.array([(yplus - yminus) / (2 * ck * di) for di in delta])

        new_params -= ak*grad

        new_energy = fun(new_params, hamiltonian, return_grad=False)
...
def adam(fun, x0, maxiter=100, stepsize=0.01, beta1=0.9, beta2=0.99, tol=1e-4, eps=1e-08, verbose=False, demon=False, adamax=False, weight_decay=0, hamiltonian=None, **options):
    new_params = np.array(x0)
    ref_energy, grad = fun(new_params, hamiltonian)
